...
const compress = async (file: File, options: VideoMetadata): Promise<string> => {
    ...
    const result = await apiFetch<APIResponse>(API_URL + '/api/v1/media/compress', {
        method: 'POST',
        body: formData,
    });
...
const saveSection = async (startTime: string, endTime: string, title?: string, description?: string): Promise<string> => {
    const result = await apiFetch<APIResponse>(API_URL + '/api/v1/media/save', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ startTime, endTime, title, description }),
    });

    if (result.status === 'error') throw new Error(`Failed to save section: ${result.message}`);

    return result.data.uuid;
};

const saveSectionByMarkers = async (startMarkerId: number, endMarkerId: number, title?: string, description?: string): Promise<string> => {
    const result = await apiFetch<APIResponse>(API_URL + '/api/v1/media/save/markers', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
const clipSection = async (duration: number, title?: string, description?: string): Promise<string> => {
    const result = await apiFetch<APIResponse>(API_URL + '/api/v1/media/clip', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ duration, title, description }),
    });

    if (result.status === 'error') throw new Error(`Failed to clip section: ${result.message}`);
